...
def load_uciml_by_id(dataset_id: int, cache_dir: pathlib.Path) -> Tuple[DatasetSpec, pd.DataFrame, Optional[pd.Series]]:
    ...
    if y is None:
        ...
        spec.target = y.columns[0] if hasattr(y, "columns") and len(y.columns) else y.name
